...
from utils.gui_helpers import set_icon, set_window_constraints, center_window
from utils.summarize import run_summarizer
from utils.theme import get_current_theme, apply_theme, toggle_theme_util, apply_toggle_style
...
class SummarAIzeGUI(tk.Tk):
    def __init__(self):
        ...
        #set_window_dimensions(self, False)
        
        # Load settings and set window size 
        self.settings = load_settings()
        width = self.settings["Options"].get("width", 800)
        height = self.settings["Options"].get("height", 600)
        self.geometry(f"{width}x{height}")
        
        # Apply Azure theme [dark & light themes]
        apply_theme(self, get_current_theme(self.settings))
        apply_toggle_style(get_current_theme(self.settings), ttk)
        ...
        # Save window dimensions after each resizing
        self.bind("<Configure>", self._on_resize)

        center_window(self)
        #self.start_debug_resize_watcher()

    # Main GUI widget creation
    def create_widgets(self):
        ...
        self.output_box = ThemedScrolledText(self.output_box_frame, wrap="word", theme=self.current_theme())
    ...
    # handles saving window dimensions on resize
    ...
    # handles ANSI in output content box
    def _init_ansi_tags(self):
        """Create a text‑tag for every colour in ANSI_COLOR_MAP."""
        for esc, name in ANSI_COLOR_MAP.items():
            if name == "reset":
                continue
            # basic colour names work fine; customise for themes if you wish
            self.text.tag_config(name, foreground=name)
    def insert_ansi(self, line: str):
        """
        Insert a line that may contain ANSI escapes,
        colouring each segment with the correct tag.
        """
        current_tag = None
        for part in ANSI_PATTERN.split(line):
            if part in ANSI_COLOR_MAP:           # it's an escape sequence
                tag_name = ANSI_COLOR_MAP[part]
                current_tag = None if tag_name == "reset" else tag_name
            else:
                self.text.insert(tk.END, part, current_tag)
        self.text.insert(tk.END, "\n")
        self.text.see(tk.END)

    # handle returning the current theme
    def current_theme(self):
        return "dark" if self.is_dark_theme.get() else "light"
    # handle toggling the theme either dakr or light
    def toggle_theme(self):
        ...
        self.output_box.update_theme(self.current_theme())
    
    # handles when you toggle output to file or not
    def toggle_output(self):

        self.update_idletasks()

        set_window_constraints(self, self.save_to_file.get())
    ...
    # handles selecting the input file
    ...
    # handles selecting the output file [text only for now but we can change this later]
    ...
    # handles calling the cli but with gui mode flag [THREADED]
    def summarize(self):
        model = self.model_choice.get()
